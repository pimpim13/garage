...
class ModeleSeanceUpdateView(GestionnaireRequiredMixin, UpdateView):
    model = ModeleSeance
    form_class = ModeleSeanceForm
    template_name = 'scheduling/modele_form.html'
    ...
    def form_valid(self, form):
        messages.success(self.request, "Séance type modifiée.")
        return super().form_valid(form)


class ModeleSeanceDeleteView(GestionnaireRequiredMixin, DeleteView):
    model = ModeleSeance
    template_name = 'scheduling/modele_confirm_delete.html'
    context_object_name = 'modele'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['nb_seances'] = self.object.seances.count()
        return context
    ...
    def form_valid(self, form):
        messages.success(self.request, "Séance type supprimée.")
        return super().form_valid(form)
